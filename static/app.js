...
function handleImageError(reportId, fallbackUrl) {
  const container = document.getElementById('img-' + reportId);
  if (!container) return;

  // If we have a Discord fallback URL, try loading from there
  if (fallbackUrl && fallbackUrl.startsWith('http')) {
    const img = container.querySelector('img');
    if (img && !img.dataset.triedFallback) {
      img.dataset.triedFallback = 'true';
      img.src = fallbackUrl;
      img.onclick = () => openImageModal(fallbackUrl);
      return; // Try Discord CDN
    }
  }

  // Both local and Discord failed - show error
  container.innerHTML = `
    <div class="report-card-image-error">
      ...
      ${fallbackUrl ? `<a href="${fallbackUrl}" target="_blank" onclick="event.stopPropagation();">Otwórz link</a>` : ''}
    </div>
  `;
}
